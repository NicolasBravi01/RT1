...
'''
while 1:
    dist, rot_y = find_token_color(MARKER_TOKEN_SILVER)  # we look for markers
    if dist == -1:
...
def take_color (color):
	...
	while(not(token)):
		distance, angle = find_token_color(color)
		print(distance, angle)
		
		while(distance == -1 and angle == -1):
			turn(-50, 0.1)
			distance, angle = find_token_color(color)
			
		if (distance>d_th or angle >abs(a_th)):		
			driveTo(distance, angle)
		else:
			R.grab()
			drive(30, 1.5)
			turn(-30, 0.5)
...
def main():	
	...
	take_color(MARKER_TOKEN_SILVER)
	take_color(MARKER_TOKEN_GOLD)
